...
if __name__ == "__main__":
    ...
    try:
        ...
        for log_index, log_file_path in enumerate(logs_selected, start = 1):
            ...
            log_file_date_str: str = log_file_date.strftime("%Y%m%d")
            log_file_next_date: date = log_file_date + timedelta(days = 1)
            ...
            db_util.execute_sql("""
                DELETE FROM stg.postgre_log_copy
                WHERE LOWER(TRIM(database_name)) != 'mgdb';
            """)
            ...
            db_util.execute_sql(f"""
                CREATE TABLE IF NOT EXISTS landing.postgre_log_{log_file_date_str}
                PARTITION OF landing.postgre_log
                FOR VALUES FROM ('{log_file_date}') TO ('{log_file_next_date}');
            """)
            ...
            db_util.execute_sql(f"""
                CREATE TABLE IF NOT EXISTS landing.pgaudit_session_log_{log_file_date_str}
                PARTITION OF landing.pgaudit_session_log
                FOR VALUES FROM ('{log_file_date}') TO ('{log_file_next_date}');
            """)
